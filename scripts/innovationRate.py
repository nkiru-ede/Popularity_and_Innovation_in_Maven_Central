...
start_year = 2002
end_year = 2022  # Exclude 2023 and 2024
ga_counts_filtered = ga_counts[(ga_counts['year'] >= start_year) & (ga_counts['year'] <= end_year)]

plt.figure(figsize=(10, 6))

plt.plot(
    ga_counts_filtered['year'], 
    ga_counts_filtered['Innovation1'], 
    marker='o', linestyle='-', color='purple', 
    label='Innovation1 (FirstGA/LastGA)'
)
plt.plot(
    ga_counts_filtered['year'], 
    ga_counts_filtered['Innovation2'], 
    marker='o', linestyle='-', color='orange', 
    label='Innovation2 (MajorReleaseGA/LastGA)'
)
...
# Set x-axis ticks to even years
