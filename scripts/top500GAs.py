...
def process_artifacts(file_path):
    ...
    df['dependency_release_year'] = df['source_release'].str[:4]

    df['aggregated_artifact'] = df['target'].str.split(':').str[:2].str.join(':')

    artifact_dependency_counts = df.groupby(
        ['aggregated_artifact', 'dependency_release_year']
    )['Dependencies'].count().reset_index()

    artifact_dependency_counts.rename(columns={'Dependencies': 'dependency_count'}, inplace=True)

    artifact_dependency_counts = artifact_dependency_counts.sort_values(
        by=['dependency_release_year', 'dependency_count'], ascending=[True, False]
    )
